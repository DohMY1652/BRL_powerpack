...
def get_datas_from_bag(
    ...
    bag = rosbag.Bag(f'bag/{bag_file}.bag')

    topics = list(bag.get_type_and_topic_info().topics.keys())
    topics.remove('/rosout')
    if '/rosout_agg' in topics:
        topics.remove('/rosout_agg')

    # generate data from bag file
    data = {key: {'time_stamp': deque(), 'data': deque()} for key in topics}
    for topic, msg, time_stamp in bag.read_messages(topics=topics):
        data[topic]['time_stamp'].append(time_stamp.to_sec())
    ...
    for msg in data[topic]['data']:
        topic_data['time'].append(msg.time)
        topic_data['sen_pos'].append(msg.sen_pos)
        topic_data['sen_neg'].append(msg.sen_neg)
        topic_data['ref_pos'].append(msg.ref_pos)
        topic_data['ref_neg'].append(msg.ref_neg)
        topic_data['ctrl_pos'].append(msg.ctrl_pos)
        topic_data['ctrl_neg'].append(msg.ctrl_neg) 

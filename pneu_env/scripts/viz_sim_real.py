...
def plot_datas(datas, df, save_name=None):
    ...
    ax1.plot(
        ...
        np.array(df["curr_time"]),
        np.array(df["press_pos"]),
...
if __name__ == '__main__':
    ...
    df = pd.read_csv(f"{get_pkg_path('pneu_env')}/exp/{exp_name}")
    ...
    if data_log == '1':
        save_name = f'{formatted_time}_SimVsReal_{obs_type}'
    else:
        save_name = None
    ...
    obs.set_init_press(
        init_pos_press = df["press_pos"].values[0],
        init_neg_press = df["press_neg"].values[0]
    )
    ...
    try:
        ...
        while curr_time < ctrl.max_time:
            ...
            curr_obs, info = obs.observe(action)

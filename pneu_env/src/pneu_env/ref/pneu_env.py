...
class PneuEnv():
    def __init__(
        ...
        self.obs_buf = 101.325*np.ones(self.dim_obs_set*step_obs, dtype=np.float64)
        obs_idxs = deque()
        for i in range(num_obs):
            obs_idxs.append(2*i*step_obs) 
            obs_idxs.append(2*i*step_obs + 1) 
    ...
    def get_state(
        ...
        sen_per_ctrl = int(self.sen_freq/self.ctrl_freq)

        goal = self.goal.get_ref(self.time_buf)
        for _ in range(sen_per_ctrl):
    ...
    def get_reward(
        ...
        reward = press_reward + act_reward + coup_reward
    ...
    def verbose(
        ...
        print(
            ...
            f'\t      Coup\t{info["reward"]["coup_reward"]}'
        )
        for _ in range(10):

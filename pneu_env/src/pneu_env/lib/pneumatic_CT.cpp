...
PneumaticCT::PneumaticCT()
{
    ...
    std::cout << "[ INFO] CPOS : " << CPOS << " CNEG : " << CNEG << std::endl;
...
double* PneumaticCT::model(double* x, double* u)
{
    ...
    double mdot_pos_valve = this -> solenoid_valve(Ppos, ATM, pos_valve, CPOS);
    double mdot_neg_valve = this -> solenoid_valve(ATM, Pneg, neg_valve, CNEG);

    double leakage_pos = this -> leakage(Ppos, ATM, CLPOS);
    double leakage_neg = this -> leakage(ATM, Pneg, CLNEG);
    // double leakage_pos = 0;
    // double leakage_neg = 0;
    double mdot_out = dm1outdt + dm2outdt - mdot_pos_valve - leakage_pos;
    double mdot_in = - dm1indt - dm2indt + mdot_neg_valve + leakage_neg;
    ...
    mass_flowrate[1] = dm1indt + dm1indt;
    mass_flowrate[2] = leakage_pos;
    mass_flowrate[3] = leakage_neg;
...
double PneumaticCT::leakage(double P_inlet, double P_outlet, double Cd)
{
    double dmdt;
...
double PneumaticCT::solenoid_valve(double P_inlet, double P_outlet, double ratio, double Cd)
{
    double dmdt;
    ...
    {
        if (Pout/Pin <= Pcr) {
            dmdt = PI*D*ratio*d * (Pin/sqrt(1000*R*T_OUT)) * sqrt(K*pow(2/(K + 1), (K + 1)/(K - 1)));
        } else {
            dmdt = PI*D*ratio*d * (Pin/sqrt(1000*R*T_OUT)) * sqrt(2*K/(K - 1)) * sqrt(pow(Pout/Pin, 2/K) - pow(Pout/Pin, (K + 1)/K));
        }
    } else {
        dmdt = 0;
    }
